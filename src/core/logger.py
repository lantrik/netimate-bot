...
def setup() -> None:
    ...
    dispatcher_log = logging.getLogger("aiogram.dispatcher")
    dispatcher_log.setLevel(_LOG_LEVEL)

    for handler in dispatcher_log.handlers:
        if isinstance(handler, logging.StreamHandler):
            dispatcher_log.removeHandler(handler)

    dispatcher_log.addHandler(_file_handler())